...
function sessionListFields(header: SessionHeader, events: readonly SessionEvent[] = []): {
  ...
  return {
    ...
    // The fork seed cut: lets a client group sibling forks that share the
    // same inherited prefix (same logical turn) into one "version family".
    ...
    workspace: {
      ...
      async deleteSession(request) {
        ...
        try {
          ...
          // Only the registry's unknown-session rejection is business code;
          // storage/durability failures propagate as internal errors.
          if (!(error instanceof WorkspaceUnknownSessionError)) throw error
          return err(request, {
            code: 'session-not-found',
