 ...
 */
export function versionRootOf(sessionId) {
...
function readVersionTreeByRoot() {
    ...
    if (Object.keys(parsedObj).some(key => !/^\d+$/.test(key))) {
        ...
        for (const [rootKey, value] of Object.entries(parsedObj)) {
            ...
            for (const [atSeqKey, v] of Object.entries(value)) {
                const t = v;
                if (t === null || typeof t !== 'object' || typeof t.original !== 'string' || !Array.isArray(t.versions))
                    continue;
                turns[atSeqKey] = {
                    original: t.original,
    ...
    if (Object.keys(flatTree).length > 0)
        return namespaceFlatTree(flatTree);
 ...
 */
export function versionFamilyOf(byId, atSeq, rootId) {
    const tree = readVersionTreeByRoot();
...
export function VersionPager({ messageId, useSession, useSessions, sessionId, openSession, t, }) {
    ...
    const family = useMemo(() => {
        if (atSeq === undefined || sessionId === undefined)
            return undefined;
