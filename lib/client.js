	...
	factory: (require) => {
		...
		*/
		function versionRootOf(sessionId) {
			...
			for (const entry of Object.values(readVersionTree())) for (const version of entry.versions) forkOriginal.set(version, entry.original);
		...
		function readVersionTree() {
			try {
				...
				for (const [atSeqKey, value] of Object.entries(parsed)) {
					...
					const t = value;
					if (typeof t.original === "string" && Array.isArray(t.versions)) tree[atSeqKey] = {
						original: t.original,
		...
		*/
		function versionFamilyOf(byId, atSeq) {
			const turn = readVersionTree()[String(atSeq)];
			if (turn === void 0) return void 0;
		...
		*/
		...
		exports.readVersionTree = readVersionTree;
